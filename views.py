...
def home_page(request):
    ...
    category_list = []
    app_categories_objs = allAppData.objects.exclude(app_category__isnull=True).exclude(app_category='null').values_list('app_category').distinct()
    for category in app_categories_objs[1:]:
        if len(allAppData.objects.filter(app_category = category[0])) >= 2:
            category_list.append(category[0])
        else:
            continue
    context = {
        "app_list": app_list,
        "app_categoris": category_list,
        "dict_data": dict_data,
...
def get_category_wise_data(request):
    if request.method == "POST":
        ...
        print(len(get_category_wise_apps))
...
def kmeans_cluster(data_list, app_name_list):
    ...
    print("---------")
    print("upper_range_values : {}".format(upper_range_values))
    print("lower_range_values : {}".format(lower_range_values))
    print("kmeaascending_centrodisns : {}".format(ascending_centrodis))
